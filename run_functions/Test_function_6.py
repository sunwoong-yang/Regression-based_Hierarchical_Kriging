...
np.random.seed(42)
LF_x = lhs(in_dim, samples=150, criterion='maximin') * 1 + 2
MF_x = lhs(in_dim, samples=100, criterion='maximin') * 1 + 2
HF_x = lhs(in_dim, samples=50, criterion='maximin') * 1 + 2
...
IHKs, RHKs, i_errors, r_errors, IHK_likeli, RHK_likeli, IHK_time, RHK_time, x_scaler = train_models([LF_x, MF_x, HF_x], [LF_y, MF_y, HF_y],
                                              test_x=test_x, test_y=ground_truth, pop=50,
...
IHKs, RHKs, i_errors, r_errors, IHK_likeli, RHK_likeli, IHK_time, RHK_time, x_scaler = train_models([LF_x, MF_x, HF_x], [LF_y, MF_y, HF_y],
                                              ...
                                              history=True, repetition=1, add_noise=[[0, 0., 0.], [1, 0., 0.]], rand_seed=42)
