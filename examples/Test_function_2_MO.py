...
from surrogate_model.HKs import HKs
...
LF_y1 = LF_function(LF_x).reshape(-1,1)
MF_y1 = MF_function(MF_x).reshape(-1,1)
HF_y1 = HF_function(HF_x).reshape(-1,1)

LF_y2 = LF_y1 + 1
MF_y2 = MF_y1 + 1
HF_y2 = HF_y1 + 1

LF_y = np.hstack((LF_y1, LF_y2))
MF_y = np.hstack((MF_y1, MF_y2))
HF_y = np.hstack((HF_y1, HF_y2))

hk = HKs(x=[LF_x, MF_x, HF_x], y=[LF_y, MF_y, HF_y], n_pop=[100,100,100], n_gen=[100,100,100], HKtype="r")
...
pred_HF_y_HK = hk.predict(X=test_x, pred_fidelity=2, return_std=False)

fig, ax = plt.subplots(dpi=300)
ax.plot(test_x, HF_function(test_x), c='k', ls='--', label="Ground truth")
ax.scatter(mfdnn.train_x[0],mfdnn.train_y[0][:,0], c='C0', label='LF')
ax.scatter(mfdnn.train_x[1],mfdnn.train_y[1][:,0], c='C1', label='MF')
ax.scatter(mfdnn.train_x[2],mfdnn.train_y[2][:,0], c='C2', label='HF')
ax.plot(test_x, pred_LF_y[:,0], c='C0', label="LF pred")
ax.plot(test_x, pred_MF_y[:,0], c='C1', label="MF pred")
ax.plot(test_x, pred_HF_y[:,0], c='C2', label="HF pred")
ax.plot(test_x, hfdnn.predict(test_x)[:,0], c='C3', label="Only HF pred")
ax.plot(test_x, pred_HF_y_HK[:,0], c='C4', label="RHK_y1")
ax.plot(test_x, pred_HF_y_HK[:,1], c='C5', label="RHK_y2")
ax.legend()
plt.show()
