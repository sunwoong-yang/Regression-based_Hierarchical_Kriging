...
IHK_Func5 = np.load("../results_functions/error/IHK_Func3.npy", )
RHK_Func5 = np.load("../results_functions/error/RHK_Func3.npy", )
IHK_Func6 = np.load("../results_functions/error/IHK_Func3.npy", )
RHK_Func6 = np.load("../results_functions/error/RHK_Func3.npy", )

error_set = ["RMSE", "MAE"]
error_name = np.array([i for i in error_set for j in range(30)]).reshape(-1, 1)
error_name = np.vstack((error_name, error_name))
IorR_name = np.array([i for i in ["IHK", "RHK"] for j in range(30 * len(error_set))]).reshape(-1, 1)
...
box_setting_IHK = dict(boxprops=dict(facecolor=current_palette[0]), patch_artist=True, medianprops=dict(color='k'), showfliers=False)
box_setting_RHK = dict(boxprops=dict(facecolor=current_palette[1]), patch_artist=True, medianprops=dict(color='k'), showfliers=False)
